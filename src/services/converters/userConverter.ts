...
const requestStatuses: JoinRequest['status'][] = ['pending', 'approved', 'declined'];
...
const childrenFromRecord = (value: unknown): Child[] =>
  Array.isArray(value)
    ? value.map(item => {
        const child = item && typeof item === 'object' ? (item as RawRecord) : {};
        return {
...
export const userFromRecord = (record: RawRecord): User => ({
  ...
  financialStatus: enumValue(record.financialStatus, financialStatuses, 'green'),
  ...
  notificationPreferences: preferencesFromRecord(record.notificationPreferences),
  currencySymbol: asString(record.currencySymbol, '₦'),
  timezone: asString(record.timezone, 'WAT'),
});
...
export const joinRequestFromSnapshot = (snapshot: DocumentSnapshotLike): JoinRequest =>
  joinRequestFromRecord(snapshotToRecord(snapshot));
